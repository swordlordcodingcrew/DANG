...
//    class TmxExtruder;
    ...
    public:
        Wavepoint() = default;
        Wavepoint(uint32_t id, const std::string& name, const std::string& type, Vector2F pos, uint32_t next_id, uint32_t duration, uint8_t orientation, uint32_t delay);

        spTwPos     getTwPos() const;
        spTwPosSpline   getTwPosSpline() const;
